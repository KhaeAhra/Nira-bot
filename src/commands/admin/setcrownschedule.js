...
module.exports = class SetCrownScheduleCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      Définit le calendrier de rotation des rôles de Crown de Calypso.
      ...
      Si vous souhaitez utiliser plusieurs valeurs pour l'une des catégories, veuillez les séparer par \`,\`.` +
        'La syntaxe des étapes est également prise en charge, par exemple : `0 */1 * * *` (toutes les heures). ' +
        'Pour le jour de la semaine, 0 et 7 peuvent représenter dimanche. ' +
        'Si vous avez besoin d aide supplémentaire pour créer votre cron, veuillez consulter ce site Web: <https://crontab.guru/#>. ' + 
        `N'entrez pas de programme pour effacer le \`programme de Crown\` actuel.
        Un \`rôle de Crown\` doit également être défini pour permettre la rotation des rôles.`,
  ...
  run(message, args) {
    ...
    let description = `The \`crown schedule\` was successfully updated. ${emojis.success}`;
    ...
    try {
      ...
      return this.sendErrorMessage(message, 0, 'Veuillez réessayer avec une expression cron valide ');
    }

    // Set minutes and seconds to 0
    const cron = crownSchedule.split(' ');
    if (cron[0] != '0') {
      description = description + `\n**Note:** Minutes change pour \`${cron[0]}\` to \`0\`.`;
      cron[0] = '0';
    }
    if (cron.length === 6 && cron[5] != '0') {
      if (description.includes('\n'))
        description = description.slice(0, -1) + `, and seconds change pour \`${cron[5]}\` to \`0\`.`;
      else description = description + `\n**Note:** Seconds change pour \`${cron[5]}\` to \`0\`.`;
      cron[5] = '0';
    } 
