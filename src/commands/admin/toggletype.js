...
module.exports = class ToggleTypeCommand extends Command {
  ...
  run(message, args) {
    ...
    if (args.length === 0 || args[0].toLowerCase() === OWNER)
      return this.sendErrorMessage(message, 0, 'Veuillez fournir un type de commande valid');
    
    const type = args[0].toLowerCase();
    
    if (type === ADMIN) return this.sendErrorMessage(message, 0, `${capitalize(ADMIN)} les commandes ne peuvent pas être désactivées`);
    ...
    if (types.includes(type)) {
      ...
      if (commands.every(c => disabledCommands.includes(c.name))) {
        ...
        description = `All \`${capitalize(type)}\` Les commandes de type ont été **activées** avec succès. ${emojis.success}`;
      ...
      // Disable type
        ...
        description = `All \`${capitalize(type)}\` Les commandes de type ont été **désactivées** avec succès. ${emojis.fail}`;
      }
    } else return this.sendErrorMessage(message, 0, 'Veuillez fournir un type de commande valide');
