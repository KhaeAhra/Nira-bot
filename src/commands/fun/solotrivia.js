...
module.exports = class SoloTriviaCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description: oneLine`
      Testez vos connaissances dans un jeu-questionnaire (vous seul pouvez répondre).
  ...
  run(message, args) {
    ...
    if (!topic) { // Pick a random topic if none given
      ...
      return this.sendErrorMessage(message, 0, `Veuillez fournir un sujet valide, utilisez ${prefix}topics pour une liste `);
    ...
    collector.on('end', () => {
      ...
      if (winner) 
        ...
        .setDescription(`Désolé ${message.member}, le temps est écoulé ! Plus de chance la prochaine fois.`)
