...
module.exports = class ChannelInfoCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description: oneLine`
      Récupère les informations sur le canal fourni.
      Si aucun canal n'est indiqué, le canal actuel sera utilisé
      `,
  ...
  run(message, args) {
    ...
    const embed = new MessageEmbed()
      ...
      .addField('Membres', `\`${channel.members.size}\``, true)
      .addField('Bots', `\`${channel.members.array().filter(b => b.user.bot).length}\``, true)
      .addField('Crée le', `\`${moment(channel.createdAt).format('MMM DD YYYY')}\``, true)
    ...
    if (channel.type === 'text') {
      ...
      embed // Voice embed
        .spliceFields(0, 1, { name: 'Channel', value: `${emojis.voice} ${channel.name}`, inline: true })
    ...
    Veuillez entrer mentionner un texte ou un canal d'annonce valide ` +
      ' ou fournissez un texte, une annonce ou un ID de canal vocal valide '
    );
