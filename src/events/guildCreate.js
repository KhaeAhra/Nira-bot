...
const colors = require('../../dataJson/colors.json');
...
module.exports = async (client, guild) => {
  ...
  if (serverLog)
    serverLog.send(new MessageEmbed().setDescription(`${client.user} a rejoint **${guild.name}** ${emojis.success}`));
  ...
  // Self-assign color
  try {
    const niraColor = guild.roles.cache.find(r => r.name === '#nira');
    if (niraColor) await guild.me.roles.add(niraColor);
  } catch (err) {
    client.logger.error(err.message);
  }
